from .scorers import (
    SimpleDirectionalScorer,
    RatioThresholdScorer,
    InverseRatioScorer,
    RatioGoalInverseScorer,
    GoalRatioScorer,
    NDGainScorer,
    PopulationDensityScorer,
)
...
class IndicatorScorerFactory:
    def __init__(self) -> None:
        ...
        self._nd_gain_scorer = NDGainScorer()
        self._population_density_scorer = PopulationDensityScorer()

    def for_series(self, series_code: str):
        if series_code in self._scorers:
            return self._scorers[series_code]
        if series_code == "nd_vulnerability" or series_code.startswith("id_"):
            return self._nd_gain_scorer
        if series_code == "EN.POP.DNST":
            return self._population_density_scorer
        raise KeyError(f"No scorer registered for series_code={series_code!r}")
